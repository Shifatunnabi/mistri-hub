...
import { 
  ArrowLeft, MessageCircle, Star, CheckCircle2, Clock, Loader2, Calendar,
...
export default function JobTimelinePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchJobDetails = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log("Help Seeker ID:", data.job.helpSeeker._id)
        console.log("Assigned Helper ID:", data.job.assignedHelper?._id)
        ...
        console.log("Is Assigned Helper:", isAssignedHelper)
  ...
  const handleSubmitReview = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setHasReviewed(true)
        // Fetch the review data to display it
        await checkIfReviewed(resolvedParams.id)
      } else {
  ...
  const handleSubmitReport = async () => {
    ...
    const reportedUserId = isJobOwner ? job.assignedHelper?._id : job.helpSeeker._id
  ...
  const reportCategories = [
    { value: "misbehave", label: "Misbehavior" },
    { value: "inexperienced", label: "Inexperienced/Unqualified" },
    ...
    <div className="min-h-screen bg-muted/30 py-8">
      <div className="container mx-auto max-w-4xl px-4">
        ...
        <div className="space-y-6">
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid gap-6 sm:grid-cols-2">
                <div className="space-y-3">
                  <h3 className="font-bold">Job Seeker</h3>
                  <Link href={`/profile/${job.helpSeeker._id}`} className="block">
                    <div className="flex items-center space-x-3 p-3 rounded-lg hover:bg-muted/50 transition-colors">
                      <Avatar className="h-12 w-12 ring-2 ring-border">
                        <AvatarImage src={job.helpSeeker.profilePhoto || "/placeholder.svg"} />
                        <AvatarFallback className="bg-primary text-primary-foreground">
                          {job.helpSeeker.name?.charAt(0) || "U"}
                        </AvatarFallback>
                      </Avatar>
                      <div>
                        <p className="font-bold">{job.helpSeeker.name || "Unknown"}</p>
                ...
                {job.assignedHelper && (
                  <div className="space-y-3">
                    <h3 className="font-bold">Assigned Helper</h3>
                    <Link href={`/profile/${job.assignedHelper._id}`} className="block">
                      <div className="flex items-center space-x-3 p-3 rounded-lg hover:bg-muted/50 transition-colors">
                        <Avatar className="h-12 w-12 ring-2 ring-border">
                          <AvatarImage src={job.assignedHelper.profilePhoto || "/placeholder.svg"} />
                          <AvatarFallback className="bg-accent text-accent-foreground">
                            {job.assignedHelper.name?.charAt(0) || "H"}
                          </AvatarFallback>
                        </Avatar>
                        <div>
                          <div className="flex items-center gap-2">
                            <p className="font-bold">{job.assignedHelper.name || "Unknown Helper"}</p>
                            <VerifiedBadge isVerified={job.assignedHelper.isVerified || false} size="sm" />
                          </div>
                          <div className="flex items-center space-x-1 text-sm">
                            <Star className="h-4 w-4 fill-yellow-400 text-yellow-400" />
                            <span>{(job.assignedHelper.averageRating || 0).toFixed(1)}</span>
                            <span className="text-muted-foreground ml-1">View Profile →</span>
                          </div>
                        </div>
                      </div>
                    </Link>
          ...
          {isJobOwner && job.status === "completed" && !hasReviewed && (
            <Card className="overflow-hidden shadow-xl animate-fade-in-up animation-delay-300 relative">
              <div className="border-b border-border bg-accent/5 p-6 flex items-start justify-between">
                <div>
                ...
                <Dialog open={isReportDialogOpen} onOpenChange={setIsReportDialogOpen}>
                  ...
                  <DialogContent>
                    <DialogHeader>
                      ...
                      <DialogDescription>
                        Please describe the issue you encountered with this job
                      </DialogDescription>
                    </DialogHeader>
          ...
          {isAssignedHelper && job.status === "completed" && (
            <Card className="p-6">
                <Dialog open={isReportDialogOpen} onOpenChange={setIsReportDialogOpen}>
                  <DialogTrigger asChild>
          ...
          {/* Show review after submission - for both help seeker and helper */}
          {(isJobOwner || isAssignedHelper) && job.status === "completed" && reviewData && (
            <Card className="p-6 space-y-4">
              ...
              <div>
                <label className="text-sm font-bold text-muted-foreground">Review</label>
                <p className="mt-2 text-base leading-relaxed bg-muted/50 p-4 rounded-lg">
                  {reviewData.comment}
                </p>
              </div>
              <div className="text-xs text-muted-foreground">
                Submitted on {new Date(reviewData.createdAt).toLocaleString()}
              </div>
            </Card>
          )}

          {/* Show simple success message if review submitted but data not loaded yet */}
          {(isJobOwner || isAssignedHelper) && hasReviewed && !reviewData && (
      ...
      {job.assignedHelper && session?.user?.id && (
        <ChatWindow 
          ...
          recipient={isJobOwner ? job.assignedHelper : job.helpSeeker}
        />
      )}
    </div>
  )
